...
class GetRmtFiles:
	...
	def get_all_files(self):
		...
		try:
			...
			rmt_files = [self.filemanip.convert_filepath_ftp_path(self.current_path + local_path_sepr + x , manip_file_path + path_sepr + x) for x in rmt_files]
			rmt_files = [(x[1].replace('//','/'),x[0]) for x in rmt_files]
			return rmt_files
